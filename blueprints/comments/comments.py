...
# access to comments, posts & users dbs (for fetching & storing data)
comments = globals.db.comments
users = globals.db.users
posts = globals.db.posts

# retrieve user commets on specific post
...
# add a new comment on specific post
...
def add_new_comment(current_user, post_id):
    ...
    posts.update_one({"_id": post_obj_id}, {"$inc": {"comments_count": 1}})
    if result.inserted_id:
        new_comment["_id"] = str(new_comment["_id"])
        if isinstance(new_comment["created_at"], datetime.datetime):
